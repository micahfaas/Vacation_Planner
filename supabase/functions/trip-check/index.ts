...
// AI usage quotas (cost guardrails). Trip check runs Haiku (<1c/call); the
// limits mainly guard against runaway abuse. Enforced before the Anthropic
// call; only successful calls are counted. Tunable here with a redeploy. See
// supabase/usage.sql + project_ai_cost_guardrails.
const FEATURE = 'trip-check';
  ...
  if (admin) {
    try {
      ...
      if (status === 'user_limit') {
        return json({ ok: false, error: "You have reached this month's Trip check limit. It resets on the 1st." });
      }
      if (status === 'global_limit') {
        return json({ ok: false, error: 'Trip check is taking a breather right now — please try again later.' });
      }
    } catch {
