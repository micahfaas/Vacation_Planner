...
// Returns [{ label, url, icon }] for a place, based on its category.
export function deepLinksFor(p) {
  const out = [];
  const cat = p.category;
  if (cat === 'staying' || cat === 'lodging') {
    out.push({ label: 'Booking', url: bookingUrl(p), icon: 'ti-building' });
    out.push({ label: 'Airbnb', url: airbnbUrl(p), icon: 'ti-home' });
  }
  if (['restaurant', 'cafe', 'bar', 'cocktail', 'attraction', 'other'].includes(cat)) {
    out.push({ label: 'TripAdvisor', url: tripadvisorUrl(p), icon: 'ti-star' });
  }
  if (cat === 'attraction') {
    out.push({ label: 'Tours', url: getYourGuideUrl(p), icon: 'ti-ticket' });
  }
  return out;
}
